...
import { ErrorStateIcon } from '../icon/icon';
...
const ErrorScreen: FunctionComponent<ErrorScreenProps> = ({ testID, variation, title, subtitle, buttons }) => {
  ...
  const accessibilityFocus = useAccessibilityFocus<TouchableHighlightRef>(announcementFinished);
        ...
        <View style={styles.bottom}>
          {buttons.map((button, index) => (
            <Button
              ...
              type={button.type ?? accentVariation === !index ? 'light' : 'default'}
              textStyle={{ bold: index === 0 && !accentVariation }}
